    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sign_in_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                // 1. create SignInTCPClient
                TCPClient sign_in_tcp_client = new TCPClient();

                // 2. set send_byte_buffer
                // page_flag(1) + 공백(1) + id + 공백(1) + pw + 공백(1) + name + 공백(1) + pet_name + 공백(1) + email
                sign_in_tcp_client.send_byte_buffer = ByteBuffer.allocate(1 + 1 + id.length() + 1 + pw.length() + 1 + name.length() + 1 + pet_name.length() + 1 + email.length());
                sign_in_tcp_client.send_byte_buffer.order(ByteOrder.LITTLE_ENDIAN);
                sign_in_tcp_client.send_byte_buffer.put("1".getBytes()); // sign in page flag : 1
                sign_in_tcp_client.send_byte_buffer.put(" ".getBytes());
                sign_in_tcp_client.send_byte_buffer.put(id.getBytes());
                sign_in_tcp_client.send_byte_buffer.put(" ".getBytes());
                sign_in_tcp_client.send_byte_buffer.put(pw.getBytes());
                sign_in_tcp_client.send_byte_buffer.put(" ".getBytes());
                sign_in_tcp_client.send_byte_buffer.put(name.getBytes());
                sign_in_tcp_client.send_byte_buffer.put(" ".getBytes());
                sign_in_tcp_client.send_byte_buffer.put(pet_name.getBytes());
                sign_in_tcp_client.send_byte_buffer.put(" ".getBytes());
                sign_in_tcp_client.send_byte_buffer.put(email.getBytes());

                // 3. start SignInTCPClient
                sign_in_tcp_client.start();
                try {
                    sign_in_tcp_client.join();
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                try {
                    receive_string = new String(sign_in_tcp_client.receive_byte, 0, sign_in_tcp_client.receive_byte_length, "UTF-8");
                ...
                sign_in_tcp_client.close_socket_and_stream();
