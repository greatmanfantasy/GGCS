    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        login_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                tcp_client.send_byte_buffer = ByteBuffer.allocate(1 + 1 + id.length() + 1 + pw.length()); // page_flag(1) + 공백(1) + id + 공백(1) + pw
                tcp_client.send_byte_buffer.order(ByteOrder.LITTLE_ENDIAN);
                tcp_client.send_byte_buffer.put("0".getBytes()); // login page flag : 0
                ...
                if (receive_string.charAt(0) == '1') { // receive login success flag from server
                    ...
                    myApp.setUserId(login_data[1]);
                    myApp.setUserPw(login_data[2]);
                    myApp.setUserName(login_data[3]);
                    myApp.setPetName(login_data[4]);
                    myApp.setUserEmail(login_data[5]);
                    Log.d(this.getClass().getName(), myApp.getUserId() + " " + myApp.getUserPw() + " " + myApp.getUserName() + " " + myApp.getPetName() + " " + myApp.getUserEmail());
        ...
        sign_in_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent sign_in_intent = new Intent(getApplicationContext(), SignIn.class);
